sap.ui.jsfragment("bin.forms.in.locations", {
                ...
                db: [
                    {
                        type: "query",
                        name: "qry1",
                        dml: "select *from locations where code=':pac'",
                        where_clause: " code=':code'",
                        update_exclude_fields: ["code", "attachment"],
            ...
            return {
                ...
                afterNewRow: function (qry, idx, ld) {
                    if (qry.name == "qry1") {
                        ...
                        // var newKf = Util.getSQLValue("select nvl(max(no),0)+1 from locations");
                        // that.frm.setFieldValue("qry1.code", newKf, newKf, true);
                    }
                },
                beforeDeleteValidate: function (frm) {
                    ...
                    if (qry.name == "qry1" && (qry.status == FormView.RecordStatus.EDIT) ||
                        ...
                        var no = that.frm.getFieldValue("qry1.code");
                        var vldtt = Util.getSQLValue(
                            "select nvl(max(ord_no),-1) from c_order1 where (" +
                            "location_code='" + no + "' ) "
                        );
                        if (Util.nvl(vldtt, -1) >= 0)
                            FormView.err("Err ! , This Location exist in delivery # " + vldtt);
                        vldtt = Util.getSQLValue(
                            "select nvl(max(invoice_no||' - '||inv_refnm||' '||invoice_date),'') from pur1 where location_code='" + no +
                            "'");
                        if (Util.nvl(vldtt, '') != '')
                            FormView.err("Found in sale/purchase transactions # " + vldtt);
                        vldtt = Util.getSQLValue(
                            "select nvl(max(ord_no||'-'||ord_refnm||' '||ord_date),'') from pord1 where location_code='" + no +
                            "'");
                        if (Util.nvl(vldtt, '') != '')
                            FormView.err("Found in SO/PO transactions #  " + vldtt);
                    }
                },
