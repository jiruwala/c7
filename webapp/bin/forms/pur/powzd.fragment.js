sap.ui.jsfragment("bin.forms.pur.powzd", {
    ...
    load_detailPageData: function () {
        ...
        if (dt.ret == "SUCCESS") {
            ...
            Util.setColProp(qv, "RCVD_PKQTY", "display_width", 90);
            ...
            Util.setColProp(qv, "RCVD_P", "display_width", 60);
            ...
            Util.setColProp(qv, "VARIA_QTY", "display_width", 90);
    ...
    loadData: function () {
        ...
        if (dt.ret == "SUCCESS") {
            ...
            qv.mLctb.parse("{" + dt.data + "}", true);
            qv.loadData();
            if (thatForm.oController.poKeyFld != undefined) {
                setTimeout(() => {
                    var ld = qv.mLctb;
                    for (var i = 0; i < ld.rows.length; i++)
                        if (ld.getFieldValue(i, "KEYFLD") == parseFloat(thatForm.oController.poKeyFld)) {
                            qv.getControl().setSelectedIndex(i);
                            thatForm.view.byId("cmdNext1").firePress();
                            thatForm.oController.poKeyFld = undefined;
                            break;
                        }
                });
            }
