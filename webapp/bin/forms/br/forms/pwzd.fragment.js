sap.ui.jsfragment("bin.forms.br.forms.pwzd", {
    ...
    createView: function () {
        ...
        this.txtRef = new sap.m.Input({
            ...
            valueHelpRequest: function (e) {
                ...
                var sqDlvCounts = "(select count(*)  from c_order1 where c_ycust.code=c_order1.ord_ref and ord_code=11 and saleinv is null and " +
                    locWhere +
                    " and ord_date>=" + Util.toOraDateString(fromdt) +
                    " and ord_date<=" + Util.toOraDateString(todt) + ")";

                var sqWhere = "(select distinct ord_ref from c_order1 where ord_code=11 and saleinv is null and " +
                    locWhere +
                    " and ord_date>=" + Util.toOraDateString(fromdt) +
                    " and ord_date<=" + Util.toOraDateString(todt) + ")";
        ...
        this.txtBranch = new sap.m.Input({
            ...
            valueHelpRequest: function (e) {
                ...
                var sq = "select brno code,b_name name from cbranch where brno in " +
                    " (select distinct ORD_DISCAMT from C_ORDER1 where ord_code=11 and SALEINV is null and " +
    ...
    load_detailPage: function () {
        ...
        var recheckprice = (this.recheckPrice.getSelected() ? 'Y' : 'N');
        var loc = this.txtLocations.getValue();
        ...
        var sq = "SELECT   o.periodcode," +
            ...
            "                 sum(tqty) tqty , " +
            "               GETAVGPRICEDLV_BUY(o.keyfld,'" + recheckprice + "') AVG_PRICE ," +
            ...
            "               o.KEYFLD," +
            "               o.ATTN," +
            "               GETAVGPRICEDLV_BUY(o.keyfld,'" + recheckprice + "') ," +
            "               o.ORD_DISCAMT";
        ...
        if (dt.ret == "SUCCESS") {
            ...
            qv.mLctb.cols[qv.mLctb.getColPos("ORD_NO")].commandLinkClick = function (obj) {
                ...
                UtilGen.execCmd("bin.forms.br.forms.pdlv formTitle=DELIVERY formType=dialog keyfld=" + kfld + " formSize=80%,90%", UtilGen.DBView, UtilGen.DBView, UtilGen.DBView.newPage, function () {
                    that.load_detailPage();
                });
            };
            ...
            if (that.recheckPrice.getSelected()) {
                for (var li = 0; li < ld.rows.length; li++) {
                    var p2 = ld.getFieldValue(li, "AVG_PRICE");
    ...
    createInfoPage: function () {
        ...
        this.txtInfoRcptDate = new sap.m.DatePicker({ width: "25%" });
        ...
        this.txtInfoRcptDate.setValueFormat(sett["ENGLISH_DATE_FORMAT"]);
        this.txtInfoRcptDate.setDisplayFormat(sett["ENGLISH_DATE_FORMAT"]);
        this.txtInfoRcptDate.setDateValue(UtilGen.parseDefaultValue("$TODAY"));
    ...
    load_infoPage: function () {
        ...
        this.infoPage.addHeaderContent(new sap.m.Title({ text: Util.getLangText("titPurWzd") + " / " + refName + " / " + bName }).addStyleClass("redText boldText"));
    ...
    generatePur: function () {
        ...
        sqp = (that.recheckPrice.getSelected() ?
            "pr:=GET_ITEM_PRICE2_BUY(x.ord_ship,x.ord_ref,x.ord_discamt,x.ord_date);" : "pr:= x.sale_price/x.pack;;");
        ...
        var sq = "declare " +
            ...
            " totamt number:=0;" +
            " " +
            " cursor ds is select o.*,it.packd, it.pack,it.unitd,it.pkaver,it.prd_dt prd_date,it.exp_dt exp_date from C_ORDER1 o,items it where o.ord_code=11 and o.ord_ship=it.reference and o.keyfld in (:txtKflds) order by o.keyfld , o.ORD_POS ;" +
            ...
            "   update ORDER1 set SALEINV=kfld,ord_flag=2,ord_amt=totamt where ord_code=11 and keyfld=x.keyfld; " +
            " end loop;" +
            ...
            "                 ORDERNO, C_CUS_NO,YEAR,NO_OF_RECIEVED,costcent,C_BRANCH_NO,RECIPT_DATE ) VALUES" +
            "                 (pcode, ploc, pinvno," +
            ...
            "                  2, sysdate, '', '', kfld,user, '', ''," +
            "                 '', '', '', '', 1,'KWD', 1 , 0," +
            "                 null, pref,'2003',0,null,pBrNo,rdate );" +
        ...
        if (dt.ret != "SUCCESS") {
            ...
            var kfld = Util.getSQLValue("select nvl(max(keyfld),-1) from pur1 where invoice_code=11");
            that.printInv(kfld);
            that.joApp.to(that.detailPage, "slide");
            that.load_detailPage();
            sap.m.MessageToast.show("Purchase Closing generated successfully !");
        }
