sap.ui.jsfragment("bin.forms.br.forms.punpost", {
        ...
        setTimeout(function () {
            var bts = [
                ...
                new sap.m.Button({
                    ...
                    press: function () {
                        Util.simpleConfirmDialog("Do you want to update this Purchase Invoice ?", function (oAction) {
                            that.updateInv();
                            that.loadData(true);
                        });
    ...
    createViewHeader: function () {
        ...
        this.inv_date = new sap.m.DatePicker({ width: "25%", editable: true });
        this.rcv_date = new sap.m.DatePicker({ width: "25%", editable: true });

        this.cust_code = new sap.m.Input({ textAlign: sap.ui.core.TextAlign.Begin, width: "25%", editable: false });
        this.cust_name = new sap.m.Text({ textAlign: sap.ui.core.TextAlign.End, width: "99%", editable: false }).addStyleClass("lightblueBG");

        this.gross_amt = new sap.m.Input({ textAlign: sap.ui.core.TextAlign.Center, width: "24%", editable: false });
        this.add_amt = new sap.m.Input({ textAlign: sap.ui.core.TextAlign.Center, width: "24%", editable: false, change: calcChange });
        this.disc_amt = new sap.m.Input({ textAlign: sap.ui.core.TextAlign.Center, width: "24%", editable: false, change: calcChange });
        this.net_amt = new sap.m.Input({ textAlign: sap.ui.core.TextAlign.Center, width: "24%", editable: false }).addStyleClass("lightblueBG");
        ...
        this.rcv_date.setValueFormat(sett["ENGLISH_DATE_FORMAT"]);
        this.rcv_date.setDisplayFormat(sett["ENGLISH_DATE_FORMAT"]);

        this.inv_date.setValueFormat(sett["ENGLISH_DATE_FORMAT"]);
        this.inv_date.setDisplayFormat(sett["ENGLISH_DATE_FORMAT"]);


        var fe = [
            Util.getLabelTxt("", "100%", "#"),
            Util.getLabelTxt("Purchase Invoice Info", "100%", "#", "violetText", "End"),
            ...
            Util.getLabelTxt("txtInvNo", "45%", "", "boldText redText"), this.invoice_no,
            Util.getLabelTxt("keyID", "45%"), this.keyfld,
            Util.getLabelTxt("txtDate", "45%"), this.inv_date,
            Util.getLabelTxt("rcptDateTxt", "45%", "", "redText"), this.rcv_date,
            Util.getLabelTxt("", "100%", "#"),
    ...
    loadData: function (pReload) {
        ...
        if (dt.ret == "SUCCESS") {
            ...
            this.rcv_date.setDateValue(null);
            this.inv_date.setDateValue(null);
            ...
            this.inv_date.setDateValue(new Date(dtx[0].INVOICE_DATE.replaceAll(".", ":")));
            this.rcv_date.setDateValue(new Date(Util.nvl(dtx[0].RECIPT_DATE, dtx[0].INVOICE_DATE).replaceAll(".", ":")));
        }
    ...
    updateInv: function () {
        ...
        var invdt = UtilGen.getControlValue(this.inv_date);
        var rdt = UtilGen.getControlValue(this.rcv_date);
        ...
        var sq = " declare  " +
            ...
            "   where keyfld=kfld ; " +
            "  " +
            "  update invunpaid set net_amt=(x.inv_amt+aamt)-damt where keyfld=x.keyfld ; " +
            "  end loop; " +
        ...
        sq = sq.replaceAll(":ctg", that.ctg.getValue())
            ...
            .replaceAll(":txtRcptDate", Util.toOraDateString(rdt))
    ...
    deleteInv: function () {
        ...
        if (Util.nvl(pdamt, 0) != 0)
            FormView.err("Found payments  on invoice !");
        Util.simpleConfirmDialog("Are you sure , you want to delete Purchase Invoice # " + invn + " ? ", function () {
            var sq = "declare " +
                " kfld number:=:kfld ;" +
                " tmpkf number; " +
                " cursor p2 is select *from pur2 where keyfld=kfld order by itempos; " +
                " cursor acdet is select *from acvoucher2 WHERE REFERKEYFLD=KFLD AND REFERCODE=11;" +
                " cursor ISS_acdet is select *from acvoucher2 WHERE REFERKEYFLD=tmpkf AND REFERCODE=13;" +
                " begin " +
                "  for x in p2 loop " +
                ...
                " C7_DECACCUSES(iss.accno); " +
                " end loop; " +
                " DELETE FROM ACVOUCHER1 WHERE REFERKEYFLD=KFLD AND refercode=11;" +
                " DELETE FROM ACVOUCHER2 WHERE REFERKEYFLD=KFLD AND refercode=11;" +
                " DELETE FROM ACVOUCHER1 WHERE REFERKEYFLD=tmpkf AND refercode=13;" +
                " DELETE FROM ACVOUCHER2 WHERE REFERKEYFLD=tmpkf AND refercode=13;" +
