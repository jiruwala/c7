sap.ui.jsfragment("bin.forms.br.forms.pdlv", {
    ...
    createView: function () {
        ...
        var dmlSq = "select o1.ord_ship,o1.ord_pos,o1.ord_packd,o1.ord_pack,o1.ord_pkqty,o1.sale_price, o1.EXP_RATE,  " +
            "nvl(o1.descr,IT.DESCR) DESCR,IT.PACKD,IT.PACK,O1.SALE_PRICE*O1.ORD_PKQTY AMOUNT," +
            ...
            form: {
                ...
                db: [
                    ...
                    },
                    {
                        ...
                        where_clause: " keyfld=':keyfld' ",
                        update_exclude_fields: ['KEYFLD', 'AMOUNT', 'NET_AMT', 'NET_PRICE'],
                        ...
                        insert_default_values: {
                            ...
                            "ORD_REF": Util.quoted(":qry1.ord_ref"),
                                ...
                                fnAddMenus: function (mnus) {
                                    mnus.push(new sap.m.MenuItem({
                                        ...
                                        press: function () {
                                            ...
                                            UtilGen.PurchaseOrderFunc.copyDetails(thatForm, '"ITEMS"', '"PORD1" "ORDER1"', rs);
                                            setTimeout(() => {
                        ...
                        eventCalc: function (qv, cx, rowno, reAmt) {
                            ...
                            for (var i = 0; i < ld.rows.length; i++) {
                                if (reAmt) {
                                    ...
                                    var price = Util.extractNumber(ld.getFieldValue(i, "SALE_PRICE"));
                                    var amt = price * pqt;
    ...
    helperFunc: {
                ...
                ord_txt_iid: {
                    ...
                    other_settings: {
                        ...
                        change: function (e) {
                            ...
                            UtilGen.Search.getLOVSearchField(sq, thatForm.frm.objs["qry1.ord_txt_iid"].obj, undefined, thatForm.frm.objs["qry1.svcsuppname"].obj);
                        },
                        valueHelpRequest: function (e) {
                            ...
                            UtilGen.Search.do_quick_search(e, this,
                                ...
                                "select code,name title from c_ycust where code=:CODE", thatForm.frm.objs["qry1.svcsuppname"].obj, undefined, undefined, btns);
                        }
            ...
            return [
                {
                    ...
                    sql: "select *from (select ord_no,ord_date,ord_reference,ord_ref,ord_refnm,keyfld,location_code from order1 o1 where " +
        ...
        getItemPrice: function (refer) {
            var thatForm = this.thatForm;
            var dt = thatForm.frm.getFieldValue("qry1.ord_date");
            var sqcnt = ("select get_item_price2_buy(:refer,':ref_code',:loc,:ord_date) from dual ")
        ...
        generateInvoice: function (obj) {
            ...
            if (thatForm.frm.objs['qry1'].status == FormView.RecordStatus.VIEW) {
                ...
                var str = "bin.forms.br.forms.pwzd formType=dialog formSize=900px,430px formTitle=Purchase_Wizard pcust=:pcust fromdate=:fromdate todate=:todate ordno=:ordno";
